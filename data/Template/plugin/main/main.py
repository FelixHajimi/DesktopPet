...
class main(Template):
    ...
    def create(self, image, mainTimer, physicsTimer, state, window):
        if self.timer is None:
            ...
            menu["关闭桌宠AI"] = menu.pop("启动桌宠AI")
            rich.print("[green]桌宠AI已启动![/]")
        else:
            self.timer.stop()
            self.timer = None
            self.lastTime = None
            ...
            self.state = 0
            state["pause"] = False
            menu["启动桌宠AI"] = menu.pop("关闭桌宠AI")
            rich.print("[red]桌宠AI已关闭![/]")
    ...
    def timeout(self, image: IMAGE, state: STATE):
        ...
        if state["postion"][0] + 128 >= state["screenWidth"] - 32:
            ...
            self.lastTime = time.time()
            state["motion"][0] -= 5
            print("桌宠被右侧一股无形的力量弹飞了")
        elif state["postion"][0] <= 32:
            self.state = 1
            self.eventTime = 5
            self.lastTime = time.time()
            state["motion"][0] += 5
            print("桌宠被左侧一股无形的力量弹飞了")
        if state["postion"][1] == state["screenHeight"] - 128:
            if self.state == 0:
                rich.print("[yellow]桌宠开始决策[/]")
                rand = random.random()
                if 0 < rand < 0.3:
                    print("桌宠开始等待")
                ...
                state["pause"] = True
                image.setFileName("./data/Template/plugin/main/move.gif")
                image.jumpToFrame(0)
                image.start()
                ...
                state["pause"] = True
                image.setFileName("./data/Template/plugin/main/pounce.gif")
                image.jumpToFrame(0)
                image.start()
                if time.time() - self.lastTime >= self.eventTime:
                ...
                state["pause"] = True
                image.setFileName("./data/Template/plugin/main/rect.gif")
                image.jumpToFrame(0)
                image.start()
                if time.time() - self.lastTime >= self.eventTime:
                    print("桌宠休息完毕")
